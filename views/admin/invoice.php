    ...
    <form class="uk-form" v-validator="form" @submit.prevent="save | valid">
            ...
            <div data-uk-margin>
                ...
                <span v-if="invoice.data.credit_for">
                    <em>{{ 'Credit for' | trans }}: </em> <a
                            :href="$url.route('admin/invoicemaker/invoice/edit', { id: invoice.data.credit_for_id })"
                            target="_blank"> <i class="uk-icon-external-link uk-margin-small-right"></i>{{ invoice.data.credit_for }}</a>
                </span>
        ...
        <div class="uk-grid" data-uk-grid-margin>
            <div class="uk-width-large-2-3">
                ...
                <iframe v-el:iframe src="<?= $iframe_src ?>" frameborder="0" class="uk-responsive-width" width="800"
                        height="1130"></iframe>
                    ...
                    <div class="uk-panel uk-panel-box">
                        ...
                        <div class="uk-form">
                            <table class="uk-table uk-table-hover uk-table-middle">
                                ...
                                <tr>
                                    <th>{{ 'PDF' | trans }}</th>
                                    <th class="pk-table-min-width-100">{{ 'Date / Ledger number' | trans }}<br>{{
                                        'Company' | trans }}
                                    </th>
                                    <th class="pk-table-min-width-100">{{ 'Invoice number' | trans }}<br>{{
                                        'Description' | trans }}
                                    </th>
                                    <th class="pk-table-width-150 uk-text-center">
                                        {{ 'Status' | trans }}
                                    </th>
                                    <th class="pk-table-min-width-100 uk-text-right">{{ 'Amount' | trans }}<br>{{ 'Open'
                                        | trans }}<br>{{ 'Dispute' | trans }}</label>
                                    </th>
                                </tr>
                                </thead>
                                <tbody>
                                <tr class="check-item" v-for="purchase_invoice in purchase_invoices">
                                    ...
                                    <td>
                                        ...
                                        <a v-if="purchase_invoice.pdf_file"
                                           :href="$url(purchase_invoice.pdf_url, {inline: 1})"
                                    ...
                                    <td>
                                        ...
                                        <small v-if="purchase_invoice.api_client !== 'master'"> - {{
                                            purchase_invoice.api_client }}</small><br>
                                        <a v-if="purchase_invoice.company_url" :href="purchase_invoice.company_url"
                                           target="_blank"><i
                                                    class="uk-icon-external-link uk-margin-small-right"></i></a>
                                    ...
                                    <td>
                                        <a :href="$url.route('admin/twinfield/purchase_invoice/edit', { id: purchase_invoice.id })">
                                            {{ purchase_invoice.invoice_number || '-' }}</a> <small
                                                v-if="purchase_invoice.siblings_count > 1">({{
                                            purchase_invoice.siblings_count }})</small>
                                        <br>
                                        {{ purchase_invoice.description }}
                                        <a v-if="purchase_invoice.shipment_id" :href="purchase_invoice.shipment_url"
                                           target="_blank"
                                    ...
                                    <td class="uk-text-center">
                                        <span class="uk-margin-small-right" :title="$trans('Exported')"
                                              data-uk-tooltip="delay:200">
                                            <span>
                                                <i v-if="purchase_invoice.exported"
                                                   class="uk-icon-check uk-icon-justify uk-text-success"></i>
                                                <i v-else class="uk-icon-times uk-icon-justify uk-text-danger"></i>
                                            </span>
                                        </span>
                                        <br><span class="uk-margin-small-right" :title="$trans('Approved')"
                                                  data-uk-tooltip="delay:200">
                                            <a @click="toggleValue('approved', purchase_invoice)">
                                                <i v-if="purchase_invoice.approved"
                                                   class="uk-icon-check uk-icon-justify uk-text-success"></i>
                                                <i v-else class="uk-icon-times uk-icon-justify uk-text-danger"></i>
                                            </a>
                                        </span>
                                        <br><span class="uk-margin-small-right" :title="$trans('Paid')"
                                                  data-uk-tooltip="delay:200">
                                            <i v-if="purchase_invoice.paid"
                                               class="uk-icon-check uk-icon-justify uk-text-success"></i>
                                            <i v-else class="uk-icon-times uk-icon-justify uk-text-danger"></i>
                                        </span>
                                    </td>
                                    <td class="uk-text-nowrap uk-text-right">
                                        ...
                                        <small v-if="purchase_invoice.siblings_count > 1">({{
                                            purchase_invoice.invoice_number_amount | currency '€ ' }})</small>
            ...
            <div class="uk-width-large-1-3">
                ...
                <div v-if="invoice.pdf_file" class="uk-form-row">
                    ...
                    <div class="uk-form-controls uk-text-right">
                        <button type="button" class="uk-button" @click="rerender">{{ 'Rerender PDF file' | trans }}
                        </button>
                    </div>
                </div>
                ...
                <div v-if="invoice.status === 'INITIAL'" class="uk-form-row">
                    ...
                    <div class="uk-form-controls uk-text-right">
                        <button type="button" class="uk-button" @click="credit">{{ 'Duplicate to credit invoice' | trans
                            }}
                        </button>
                    </div>
                </div>
                ...
                <div class="uk-form-row">
                    ...
                    <div v-if="invoice.amount_open" class="uk-grid uk-grid-small uk-margin-small-top"
                         data-uk-grid-margin>
                        <div class="uk-width-2-3">
                            <p>Open amount</p>
                        </div>
                    ...
                    <div class="uk-form-row">
                        ...
                        <div class="uk-form-controls">
                            <input id="invoice-debtor-company" name="company" class="uk-width-1-1"
                                   v-model="invoice.debtor.company"/>
                        </div>
                    </div>
                    <div class="uk-form-row">
                        <label for="invoice-debtor-name" class="uk-form-label">{{ 'Name' | trans }}</label>
                        <div class="uk-form-controls">
                            <input id="invoice-debtor-name" name="name" class="uk-width-1-1"
                                   v-model="invoice.debtor.name"/>
                        </div>
                    </div>
                    <div class="uk-form-row">
                        <label for="invoice-debtor-address_1" class="uk-form-label">{{ 'Address 1' | trans }}</label>
                        <div class="uk-form-controls">
                            <input id="invoice-debtor-address_1" name="address_1" class="uk-width-1-1"
                                   v-model="invoice.debtor.address_1"/>
                        </div>
                    </div>
                    <div class="uk-form-row">
                        <label for="invoice-debtor-address_2" class="uk-form-label">{{ 'Address 2' | trans }}</label>
                        <div class="uk-form-controls">
                            <input id="invoice-debtor-address_2" name="address_2" class="uk-width-1-1"
                                   v-model="invoice.debtor.address_2"/>
                        </div>
                    </div>
                    <div class="uk-form-row">
                        <label for="invoice-debtor-zip_code" class="uk-form-label">{{ 'Zip code' | trans }}</label>
                        <div class="uk-form-controls">
                            <input id="invoice-debtor-zip_code" name="zip_code" class="uk-width-1-1"
                                   v-model="invoice.debtor.zip_code"/>
                        </div>
                    </div>
                    <div class="uk-form-row">
                        <label for="invoice-debtor-city" class="uk-form-label">{{ 'City' | trans }}</label>
                        <div class="uk-form-controls">
                            <input id="invoice-debtor-city" name="city" class="uk-width-1-1"
                                   v-model="invoice.debtor.city"/>
                        </div>
                    </div>
                    <div class="uk-form-row">
                        <label for="invoice-debtor-country" class="uk-form-label">{{ 'Country' | trans }}</label>
                        <div class="uk-form-controls">
                            <input id="invoice-debtor-country" name="country" class="uk-width-1-1"
                                   v-model="invoice.debtor.country"/>
                        </div>
                    </div>
                    <hr/>
                    <div class="uk-form-row">
                        ...
                        <div class="uk-form-controls">
                            <input id="invoice-debtor-email" name="email" class="uk-width-1-1"
                                   v-model="invoice.debtor.email" v-validate:email/>
                            <p class="uk-form-help-block uk-text-danger" v-show="form.email.invalid">{{ 'Please enter a
                                valid email address' | trans }}</p>
                        </div>
                    </div>
                    <div class="uk-form-row">
                        <label for="invoice-debtor-phone" class="uk-form-label">{{ 'Phone' | trans }}</label>
                        <div class="uk-form-controls">
                            <input id="invoice-debtor-phone" name="debtor_phone" class="uk-width-1-1"
                                   v-model="invoice.debtor.phone"/>
                        </div>
                    </div>
                    <div class="uk-form-row">
                        <label for="invoice-debtor-debtor_id" class="uk-form-label">{{ 'Debtor ID' | trans }}</label>
                        <div class="uk-form-controls">
                            <input id="invoice-debtor-debtor_id" name="debtor_id" class="uk-width-1-1"
                                   v-model="invoice.debtor.debtor_id"/>
                        </div>
                    </div>
                    <div class="uk-form-row">
                        <label for="invoice-debtor-debtor_vat" class="uk-form-label">{{ 'VAT number' | trans }}</label>
                        <div class="uk-form-controls">
                            <input id="invoice-debtor-debtor_vat" name="debtor_vat" class="uk-width-1-1"
                                   v-model="invoice.debtor.debtor_vat"/>
                        </div>
                    </div>
                    <div class="uk-form-row">
                        <label for="invoice-debtor-debtor_coc" class="uk-form-label">{{ 'COC number' | trans }}</label>
                        <div class="uk-form-controls">
                            <input id="invoice-debtor-debtor_coc" name="debtor_coc" class="uk-width-1-1"
                                   v-model="invoice.debtor.debtor_coc"/>
                        </div>
                    </div>
                    ...
                    <div class="uk-form-row">
                        <div v-if="invoice.ext_key && !invoice.ext_key.includes('tfh.shipment')">
                            ...
                            <div class="uk-form-row">
                                <label for="invoice-shipment_id" class="uk-form-label">{{ 'Shipment ID' | trans
                                    }}</label>
                                <div class="uk-form-controls">
                                    <input id="invoice-shipment_id" name="shipment_id" class="uk-width-1-1"
                                           v-model="invoice.shipment_id"/>
                                </div>
                            </div>
                        </div>
                    </div>
