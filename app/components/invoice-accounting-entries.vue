    ...
    components: {
        ...
        datafield: {
            ...
            watch: {
                ...
                'datafield.amount + datafield.vat_code'() {
                    // this.$emit('check:balance');
                },
            },
    ...
    computed: {
        ...
        balanceAmounts() {
            const addEntryTotal = (sum, entry) => {
                const ledger = _.find(this.$parent.ledger_numbers, {number: entry.ledger_number,});
                let cent_amount = Math.round(entry.amount * 100);
                if (ledger && ledger.vat_code) {
